...
export function AddTripModal({ open, onClose, onCreated, prefillGeoName }: Props) {
  const { user } = useAuth()
  ...
  useEffect(() => {
    if (!prefillGeoName) return
    const found = COUNTRIES.find(c => c.geoName === prefillGeoName)
    if (found) {
  ...
  const reset = () => {
    setTitle(''); setCity(''); setStartDate(''); setEndDate('')
    setDescription(''); setSelectedCountry(null); setSearch(''); setError('')
    setSaving(false)
  }
        ...
        <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center p-4">
          ...
          >
            ...
            <div className="space-y-3">
              ...
              <input
                className={INPUT}
                placeholder="Trip title (e.g. Cherry Blossom Season)"
                value={title}
            ...
            <div className="mt-5 flex gap-3">
              <button onClick={handleClose} className="flex-1 py-3 rounded-xl border border-white/10 text-sm text-white/50 hover:text-white/70 hover:bg-white/[0.03] transition-all">
                Cancel
              </button>
              <button
